...
static void tx_kick(void)
{
  ...
  /* Peek into packet buffer WITHOUT advancing tail until USB accepts */
  uint16_t t = tx_tail;
  for (uint16_t i = 0; i < n; i++) {
    tx_pkt[i] = tx_ring[t];
    t = (uint16_t)((t + 1U) % CDC_TX_RING);
  }
  ...
  if (st == USBD_OK) {
    tx_tail = t; /* commit dequeue only on accept */
    /* completion via ECU_CDC_TxComplete → tx_busy=0 → next kick */
  } else if (st == USBD_BUSY) {
    g_cdc_tx_busy++;
    ser_set_err(ECU_SER_ERR_TX_BUSY);
    tx_busy = 0; /* retry later — bytes still in ring */
  } else {
    g_cdc_tx_fail++;
...
void ECU_Serial_Service(void)
{
  ...
  /* One-shot banner when USB becomes configured — proves TX path */
  {
    static uint8_t was_cfg;
    static uint8_t hello_sent;
    uint8_t cfg = usb_configured();
    if (cfg && !was_cfg)
      hello_sent = 0;
    was_cfg = cfg;
    if (cfg && !hello_sent) {
      hello_sent = 1;
      (void)ECU_Serial_Write("STRIX:READY\r\n");
    }
  }
